...
const STORAGE_KEY = 'porchquest369.browserCampaign.v5';
...
const BRIDGE_SETTINGS_KEY = 'porchquest369.backendBridge.v1';

const CONDITION_META = {
  watched: 'The hill has noticed your route. Camp clears this pressure.',
  tired: 'Your body needs care. Camp clears this and restores HP.',
  inspired: 'A helper or receipt gives your next risky roll +1.',
  marked: 'A trail sign follows you until camp clears it.',
  hidden: 'You are moving quietly through the hill paths.'
};
...
function normalize(raw) {
  ...
  return {
    id: c.id || 'browser-blackwood-hill', version: 5, name: c.name || 'Lanterns Under Blackwood Hill',
...
function loadBridge() {
  try { return { enabled: false, apiBase: API_BASE, serverCampaignId: '', status: 'Browser Oracle ready.', ...JSON.parse(localStorage.getItem(BRIDGE_SETTINGS_KEY) || '{}') }; }
  catch { return { enabled: false, apiBase: API_BASE, serverCampaignId: '', status: 'Browser Oracle ready.' }; }
}

function addLog(c, role, text) { return { ...c, story_log: [...c.story_log, msg(role, text)].slice(-80) }; }
...
export default function AppV6({ icons = {} }) {
  ...
  async function ensureServerCampaign() {
    ...
    setCampaign(serverCampaign); setBridge((b) => ({ ...b, serverCampaignId: serverCampaign.id, status: `Connected to ${serverCampaign.id}.` })); return serverCampaign.id;
  }
  ...
  async function runAction(label, localFn, path, options) { if (!bridge.enabled) { setCampaign((c) => localFn(c)); return; } setBusy(true); try { const payload = await backendCall(path, options); setBridge((b) => ({ ...b, status: `Backend ok: ${label}` })); if (!payload.campaign) setCampaign((c) => addLog(c, 'dm', `Backend completed: ${label}.`)); } catch (error) { setBridge((b) => ({ ...b, status: `Backend failed; browser fallback used. ${error.message.slice(0, 120)}` })); setCampaign((c) => localFn(c)); } finally { setBusy(false); } }
  async function testBackend() { setBusy(true); try { const payload = await json(await fetch(`${apiRoot}/api/health`)); setBridge((b) => ({ ...b, status: `Backend health ok: ${payload.service || 'api'} ${payload.version || ''}` })); } catch (error) { setBridge((b) => ({ ...b, status: `Backend not reachable: ${error.message.slice(0, 140)}` })); } finally { setBusy(false); } }
  function newCharacter() { setCampaign(defaultCampaign(playerName || 'Mikey', className)); setBridge((b) => ({ ...b, serverCampaignId: '', status: 'New browser campaign created. Backend link cleared.' })); }
  ...
  function exportSave() { const blob = new Blob([JSON.stringify(campaign, null, 2)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'porchquest369-save-v5.json'; a.click(); URL.revokeObjectURL(url); }
  ...
  return <div className="app-shell">
    <header className="hero"><div><p className="eyebrow">PorchQuest369 v0.6</p><h1>Lanterns Under Blackwood Hill</h1><p className="subtitle">Instant browser tabletop with condition tags and optional backend-powered adventure buttons.</p></div><div className="hero-card"><span>Adventure State</span><strong>{state.label}</strong><em>{state.detail}</em><span>{bridge.enabled ? 'Backend bridge enabled' : 'Browser oracle mode'}</span></div></header>
