...
const STORAGE_KEY = 'porchquest369.browserCampaign.v8';
...
const BRIDGE_SETTINGS_KEY = 'porchquest369.backendBridge.v4';
const LEGACY_BRIDGE_SETTINGS_KEYS = ['porchquest369.backendBridge.v3', 'porchquest369.backendBridge.v2', 'porchquest369.backendBridge.v1'];
...
function defaultCampaign(playerName = 'Mikey', className = 'Lantern-Seeker') {
  ...
  return normalize({ id: 'browser-blackwood-hill', version: 8, name: 'Lanterns Under Blackwood Hill', player: { name: playerName, class_name: className, ...preset }, quests: BASE_QUESTS, world_nodes: [], pending_patches: [], npcs: {}, flags: { routeVisited: [] }, conditions: [], ending: null, seed: 369, receipts: [], story_log: [msg('dm', 'Rain taps the Infinite Porch. Blackwood Hill is waiting.')] });
}
...
function addLog(c, role, text) { return { ...c, story_log: [...(c.story_log || []), msg(role, text)].slice(-140) }; }
...
export default function AppV8({ icons = {} }) {
  ...
  const exportSave = () => { const blob = new Blob([JSON.stringify(campaign, null, 2)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'porchquest369-save-v8.json'; a.click(); URL.revokeObjectURL(url); };
  ...
  return <div className="app-shell">
    <header className="hero">
      <div><p className="eyebrow">PorchQuest369 v0.7.1</p><h1>Lanterns Under<br />Blackwood Hill</h1><p className="subtitle">Hardened starter adventure with route tracker, quest celebrations, ending polish, rewards, and optional backend sync.</p></div>
      <div className="hero-card ending-card"><span>Adventure State</span><strong>{state.label}</strong><p className="muted">{state.detail}</p><em>{campaign.receipts.length} receipts • {routeVisited.length}/{ROUTE_STEPS.length} route steps</em></div>
    </header>
    <main className="grid">
      <section className="panel story">
        ...
        <div className="button-row wide"><button onClick={() => setCampaign((c) => resolveRoute(c, ROUTE_STEPS.find((s) => !routeVisited.includes(s.id)) || ROUTE_STEPS[ROUTE_STEPS.length - 1]))}>Advance Route</button><button onClick={() => setCampaign(meetNpcLocal)}>Meet NPC</button><button onClick={() => withBackend('/api/campaigns/:id/reward/draw', {}, drawRewardLocal)}>Reward</button><button onClick={() => withBackend('/api/campaigns/:id/camp', {}, campLocal)}>Camp</button><button onClick={() => withBackend('/api/campaigns/:id/finale', {}, finaleLocal)}>Finale</button></div>
      ...
      <aside className="sidebar">
        ...
        <section className="panel"><h2>Save</h2><div className="button-row"><button onClick={exportSave}>Export</button><label className="file-button">Import<input type="file" accept="application/json" onChange={importSave} /></label></div><p className="muted">Browser save v8. Older saves migrate forward automatically.</p></section>
      </aside>
    </main>
  </div>;
}
