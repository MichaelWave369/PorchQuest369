...
export default function AppV24() {
  ...
  const readySteps = [
    ...
    { label: 'Export review receipts', done: review.status === 'ready' && lastExport !== 'not exported this session' }
  ];
  ...
  function trackedDownload(name, data) { downloadJson(name, data); setLastExport(`${name} at ${new Date().toLocaleTimeString()}`); }
  ...
  function play(kind, card) { setTranscript((old) => [{ at: new Date().toISOString(), kind, id: card.id, title: card.title || card.id, note: card.text || card.clue || card.reward || 'playtest step' }, ...old].slice(0, 20)); }
  ...
  function loadReviewed(item) { fetch(item.path).then((r) => r.json()).then((data) => { setPack(normalize(data)); setSelectedPack(item); setTranscript([]); setLastExport('not exported this session'); setTab('overview'); }).catch(() => {}); }

  return <div className="app studio-v24">
    <header className="v22-hero v23-hero v24-hero">
      <div><p className="eyebrow">PorchQuest369 v0.9.3</p><h1>Creator Flow Studio</h1><p>Action-ready overview, playtest momentum, route clarity, and one current dashboard.</p></div>
