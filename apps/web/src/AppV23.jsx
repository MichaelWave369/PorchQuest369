...
function normalize(raw) {
  ...
  return { ...STARTER, ...source, quests: Array.isArray(source.quests) ? source.quests : [], scenes: Array.isArray(source.scenes) ? source.scenes : [], npcs: Array.isArray(source.npcs) ? source.npcs : [], rewards: Array.isArray(source.rewards) ? source.rewards : [], edges: Array.isArray(source.edges) ? source.edges : [] };
}
function slug(value, fallback) { return String(value || fallback || 'item').toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_|_$/g, '').slice(0, 64) || fallback; }
function idSet(list) { return new Set((list || []).map((item) => item.id).filter(Boolean)); }
function dupes(list) { const seen = new Set(); const d = new Set(); (list || []).forEach((x) => { if (x.id && seen.has(x.id)) d.add(x.id); seen.add(x.id); }); return Array.from(d); }
...
function Field({ label, value, onChange, area }) { return <label>{label}{area ? <textarea value={value || ''} onChange={(e) => onChange(e.target.value)} /> : <input value={value || ''} onChange={(e) => onChange(e.target.value)} />}</label>; }
...
export default function AppV23() {
  ...
  const [fixtures, setFixtures] = useState(FIXTURE_FALLBACK.map((f) => ({ ...f, detail: `${f.detail} (fallback ready)` })));
  ...
  useEffect(() => {
    ...
    ]).then(setFixtures).catch(() => setFixtures(FIXTURE_FALLBACK));
  }, []);
  ...
  function removeCard(kind, index) { setPack((old) => ({ ...old, [kind]: old[kind].filter((_, i) => i !== index) })); }
  function importJson() { const next = normalize(parse(jsonText)); setPack(next.id ? next : STARTER); }
  function loadReviewed(item) { fetch(item.path).then((r) => r.json()).then((data) => { setPack(normalize(data)); setSelectedPack(item); setTranscript([]); setTab('overview'); }).catch(() => {}); }
  ...
  function play(kind, card) { const entry = { at: new Date().toISOString(), kind, id: card.id, title: card.title || card.label || card.id, note: card.text || card.clue || card.reward || card.label || 'playtest step' }; setTranscript((old) => [entry, ...old].slice(0, 20)); }
  ...
  function moveEdge(index, dir) { setPack((old) => { const next = [...old.edges]; const target = index + dir; if (target < 0 || target >= next.length) return old; [next[index], next[target]] = [next[target], next[index]]; return { ...old, edges: next }; }); }

  return <div className="app studio-v23">
    <header className="v22-hero v23-hero"><div><p className="eyebrow">PorchQuest369 v0.9.2</p><h1>Creator Flow Studio</h1><p>Reliable fixtures, quick playtest receipts, cleaner route graph, and one current dashboard.</p></div><div className={`hero-badge ${review.status}`}><span>{review.status === 'ready' ? 'Promotion Ready' : review.status === 'review' ? 'Needs Review' : 'Blocked'}</span><b>{digest}</b></div></header>
