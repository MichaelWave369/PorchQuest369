...
const PLAYER_KEY = 'porchquest369.playerRun.v4';
...
const STARTER = {
  ...
  scenes: [
    ...
    },
    {
      ...
      quest_id: 'q_side_1',
      reward: 'trail clue',
      text: 'A soft path bends toward a small helper sign.'
    },
    {
      id: 'lantern_window',
      act: 'II',
      ...
      quest_id: 'q_main_1',
      reward: 'window clue',
      text: 'A warm square of glass blinks like it remembers you.'
    }
  ],
  npcs: [
    {
  ...
  rewards: [
    {
      id: 'blue_thread',
      title: 'Blue Thread',
      text: 'Progress the key route.',
      items: ['blue thread'],
      hp: 0,
      quest_id: 'q_main_1',
      clue: 'Blue thread points toward the key route.',
      add: [],
      clear: []
    }
  ],
    ...
    log: [
      {
        ...
        type: 'start',
        text: `${hero.name} steps onto ${pack.title}.`
      }
    ]
  };
}
...
export default function AppV29() {
  ...
  const routeEdges = useMemo(() => {
    return (pack.edges || []).filter((edge) => edge.from === currentScene?.id && findById(pack.scenes, edge.to));
  }, [pack, currentScene]);
  const fallbackEdges = useMemo(() => {
    ...
    return (pack.scenes || [])
      ...
      .map((scene) => ({
        ...
        to: scene.id,
        label: 'explore scene',
        condition: 'linear fallback'
      }));
  }, [pack.scenes, routeEdges.length, currentScene]);
  const availableRoutes = routeEdges.length ? routeEdges : fallbackEdges;
  ...
  const graphHealth = useMemo(() => {
    ...
    if (!pack.edges.length) return { tone: 'warn', status: 'linear fallback', detail: 'No authored route edges yet. Player Mode will create safe fallback choices.' };
    const broken = pack.edges.filter((edge) => !findById(pack.scenes, edge.from) || !findById(pack.scenes, edge.to));
    if (broken.length) return { tone: 'warn', status: 'edge warning', detail: `${broken.length} route edge(s) point to missing scenes.` };
    return { tone: 'good', status: 'authored route', detail: `${pack.edges.length} authored route edge(s) ready.` };
  }, [pack]);
  ...
  useEffect(() => {
    if (run) localStorage.setItem(PLAYER_KEY, JSON.stringify(run));
  }, [run]);
  ...
  function rollScene() {
    ...
    writeReceipt(
      'scene',
      `${currentScene.title}: rolled ${roll} + ${run.hero?.bonus || 0} = ${total} vs DC ${dc}. ${success ? 'Success.' : 'Partial success.'}`,
      {
  ...
  function askHelper() {
    if (!run || !relatedNpc || run.ended) return;
  ...
  function drawReward() {
    if (!run || !relatedReward || run.ended) return;
  ...
  function endAdventure() {
    if (!run) return;
    writeReceipt('ending', `${run.hero?.name || 'Hero'} ended the session at ${currentScene?.title || 'the route'}.`, {
      ended: true
    });
  }
  ...
  function loadFreshStarter() {
    fetch('content-packs/blackwood-starter.route-pack.json')
      ...
      .then((data) => {
        ...
        openLobby('Fresh reviewed starter loaded. Pick a hero and start when ready.');
      })
      .catch(() => {
        setPack(STARTER);
        openLobby('Local starter loaded. Pick a hero and start when ready.');
      });
  }
  ...
  return <div className="app player-v1 player-v101 player-v102 player-v103">
    <header className="player-hero">
      <div>
        <p className="eyebrow">PorchQuest369 v1.0.3</p>
    ...
    <nav className="player-actions">
      ...
      <button onClick={loadFreshStarter}>Load Fresh Starter</button>
      <button onClick={() => setMode('studio')}>Open Creator Studio</button>
      {run && <button onClick={() => downloadJson(`${pack.id}.adventure-receipt.json`, receipt())}>Export Adventure Receipt</button>}
    </nav>
    ...
    {!run && <section className="player-grid lobby-grid">
      ...
      <article className="panel hero-select">
        ...
        <div className="selection-summary">
          ...
          <small>{pack.scenes.length} scenes · {pack.edges.length} route edges · {graphHealth.status}</small>
        </div>
      ...
      <article className="panel pack-select">
        ...
        <div className="gallery-mini current">
          <b>{pack.title}</b>
          <span>Current loaded pack · {pack.scenes.length} scenes · {pack.edges.length} route edges</span>
    ...
    {run && <section className="player-grid active-run">
      <article className="panel table-status">
        ...
        <p>{run.ended ? 'Session ended. Export your receipt or restart when ready.' : 'Take the table actions from left to right, then choose a route.'}</p>
        <div className="turn-track">
          ...
          <span className={actionProgress(run.turn_actions) >= 3 ? 'done' : ''}>4 Route</span>
        </div>
      </article>
      ...
      <article className="panel scene-panel">
        ...
        <div className="table-buttons">
          ...
          <button className={run.turn_actions?.helper ? 'complete' : ''} onClick={askHelper} disabled={!relatedNpc || run.ended}>Ask {relatedNpc?.title || 'Helper'}</button>
          <button className={run.turn_actions?.reward ? 'complete' : ''} onClick={drawReward} disabled={!relatedReward || run.ended}>Draw Reward</button>
        </div>
      </article>

      <article className="panel route-panel">
        <h2>Choose Route</h2>
        <p>{routeEdges.length ? 'Authored route choices are available.' : 'No authored edge from here. Safe fallback choices are shown.'}</p>
        <div className="route-choice-grid">
          {availableRoutes.map((edge) => {
            const next = findById(pack.scenes, edge.to);
            return <button key={edge.id} onClick={() => travel(edge)} disabled={run.ended}>
              <b>{edge.label || 'next'}</b>
              <span>{currentScene?.title || edge.from} → {next?.title || edge.to}</span>
              {edge.condition && <small>{edge.condition}</small>}
            </button>;
          })}
          {!availableRoutes.length && <button onClick={endAdventure} disabled={run.ended}>End Session Here</button>}
        </div>
      </article>
      ...
      <article className="panel receipt-panel wide-panel">
        <div className="panel-title-row">
          ...
          <div className="button-row">
            ...
            <button onClick={() => downloadJson(`${pack.id}.adventure-receipt.json`, receipt())}>Download</button>
          </div>
        </div>
