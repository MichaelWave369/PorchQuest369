...
const PLAYER_KEY = 'porchquest369.playerRun.v2';
const STARTER = {
...
export default function AppV27() {
  ...
  const fallbackEdges = useMemo(() => {
    ...
    return (pack.scenes || []).filter((scene) => scene.id !== currentScene.id).map((scene) => ({ id: `fallback-${scene.id}`, from: currentScene.id, to: scene.id, label: 'explore scene', condition: 'fallback route' }));
